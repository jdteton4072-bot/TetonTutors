...
> **v0.3 change (founder decision).** Platform: **Railway** replaces Vercel for hosting and **Railway Postgres** replaces Supabase; **Cloudflare** (existing account) fronts the app — DNS, CDN, and WAF proxying to Railway — and supplies **R2** object storage when item figures outgrow app assets. Because Supabase also supplied auth, this pulls in one consequential swap: **Better Auth** (self-hosted, in our own Postgres via the Drizzle adapter) replaces Supabase Auth. Authorization enforcement moves to the app's data-access layer (the database is reachable only by the app server now — there is no client-facing DB API); the Phase 1 database work carries over unchanged because its migrations were written for vanilla Postgres, and the integrity triggers (append-only events, protected profile columns) plus the matrix tests remain the DB-level safety net. Sections 2–5 unchanged.
...
> **DECIDED — stack (v0.3, agent-optimized).** **Next.js (App Router) full-stack TypeScript monolith**, deployed on **Railway**. **Railway Postgres** (vanilla Postgres) as the one database. **Better Auth** for authentication — self-hosted in that same Postgres via its Drizzle adapter, email+password, sessions in our own tables. **Drizzle ORM** for typed schema and migrations. **KaTeX** for math rendering. Nightly calibration runs as a **Railway cron service** invoking a script against the same database — no queues, no microservices, no second database. **Cloudflare** (existing account) provides DNS, CDN, and WAF in front of Railway, and **Cloudflare R2** is the designated object store when item figures outgrow app assets. Hosting ≈ $10–30/mo (Railway app + Postgres + cron; Cloudflare free tier; Metabase OSS pointed at the same Postgres).
>
...
Item content is structured JSON in Postgres JSONB (stem, choices, LaTeX strings; item figures served as app assets for the MVP, moving to Cloudflare R2 when volume demands it). No CMS. No document store. One database backs product, auth, calibration, and the Metabase analytics dashboard.
